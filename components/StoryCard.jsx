...
const StoryCard = ({ post, handleTagClick, handleEdit, handleDelete }) => {
  ...
  // console.log(session.user.id, pathName, post.creator._id);
      ...
      <div>
        ...
        {post.tag.split(",").map((tag, index) => (
          <span
            ...
            className="my-4 font-satoshi text-sm text-blue-700"
          >
            #{tag.trim()}{" "}
          </span>
        ))}
        {session?.user.id === post?.creator?._id && pathName === "/profile" && (
          <div
            className="
          mt-5 flex-center gap-4 border-t border-gray-100 pt-3"
          >
            <p
